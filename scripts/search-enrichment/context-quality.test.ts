...
import cases from './context-regressions.json';
it('preserves context rollout sources with explicitly unlocked fencing collections',async()=>{
 ...
 try{
  resetSearchIndex();await whenSearchComplete();const visible=searchableReportSlugs(collections.filter(c=>c.hema).map(c=>c.slug),'all');
