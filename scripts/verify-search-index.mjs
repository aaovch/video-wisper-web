...
if (!requested) {
	console.error('Usage: npm run verify-search-index -- <report-slug|--all>');
	process.exit(2);
}
...
function verify(slug) {
	const report = JSON.parse(readFileSync(join(reportsDir, `${slug}.json`), 'utf8'));
...
if (requested === '--all') console.log(`search-index ALL PASS: ${slugs.length} reports`);
